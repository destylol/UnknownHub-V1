do -- Main
    ...
    do
        ...
        do -- clicking section
            ...
            tFarm:AddButton('Unlock x2 click boost', function()
        ...
        do -- pets and rebirths
            ...
            local luckB = tP:AddButton('Unlock x2 luck boost', function()
                plr.Boosts.DoubleLuck.isActive.Value = true
            end)
            luckB:AddTooltip('Might be visual, not sure')
            tP:AddDivider()
            tP:AddToggle('autoshiny', {
                Text = 'Auto craft to shiny all',
                Default = false
            })
            tP:AddToggle('autogolden', {
                Text = 'Auto craft to golden all',
                Default = false
            })
            tP:AddToggle('auto_best', {
            ...
            tB:AddDropdown('rebirth', {
                Values = giveRebirths(),
                Default = '~~~',
                Multi = false,
                Text = 'Rebirths'
            })
            tB:AddToggle('auto_rebirth', {
                Text = 'Auto rebirth',
                Default = false
            })
            tB:AddInput('inf_rebirth', {
                Default = '~~~',
                Numeric = true,
                Finished = false
            })
            tB:AddToggle('auto_inf_rebirth', {
                Text = 'Auto inf. rebirth',
                Default = false
            })
        end
        do -- teleport
            tTP:AddDropdown('zone', {
                Values = giveZones(),
                Default = '~~~',
                Multi = false,
                Text = 'Zones'
        ...
        do -- Misc.
            tMisc:AddButton('Collect all codes', function()
                local codesTable = {'150KCLICKS', '125KLUCK', '100KLIKES', '75KLIKES', '50KLikes', '30klikes',
                                    ...
                                    '250KLIKECLICKS', '275K2XSHINY', '300SHINYCHANCE', '300DOUBLELUCK', '325CLICKS2'}
                for _, v in pairs(codesTable) do
                    rep.Events.Client.useTwitterCode:InvokeServer(v)
                end
            end)
            tMisc:AddButton('Unlock auto clicker gamepass', function()
                Gamepass.Value = Gamepass.Value .. ';autoclicker;'
            end)
            tMisc:AddButton('Unlock auto rebirth gamepass', function()
                Gamepass.Value = Gamepass.Value .. ';autorebirth;'
            end)
        end
        do -- Credits
            tCredits:AddLabel('Script: Trustsense')
            tCredits:AddLabel('Hot library: wally')
            tCredits:AddLabel('Additional help: Discord!0000')
        end
        do
    ...
    rs.RenderStepped:connect(function()
        if Toggles.autoclick.Value then
            getsenv(plr.PlayerGui.mainUI.HUDHandler).activateClick()
        end
        ...
        if Toggles.auto_rebirth.Value then
            rebMod.requestRebirth(tonumber(Options.rebirth.Value))
        end
        if Toggles.auto_inf_rebirth.Value then
            rebMod.requestRebirth(tonumber(Options.inf_rebirth.Value))
        end
